...
Эти же папки использованы в настройках PMS для фильмов и сериалов. Сразу после закачки, автоматически запускается сбор метаданных, а затем запускается данный агент (см. Настройки)
...
- обзоры с кинопоиска
- сломалась load_gallery - починил, переписал на неклассовую функцию
...
Плагин в активной доработке, практически почти каждый коммит автоматически генерит новую версию, поэтому в настройках (см.ниже) рекомендую включить автообновление.
